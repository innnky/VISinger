...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
	...
	for batch_idx, (phonemes, phonemes_lengths,
		...
		time_1 = time.time()
		phonemes, phonemes_lengths = phonemes.cuda(rank, non_blocking=True), phonemes_lengths.cuda(rank, non_blocking=True)
		notepitch, notepitch_lengths = notepitch.cuda(rank, non_blocking=True), notepitch_lengths.cuda(rank, non_blocking=True)
		...
		slurflag, slurflag_lengths = slurflag.cuda(rank, non_blocking=True), slurflag_lengths.cuda(rank, non_blocking=True)
		...
		with autocast(enabled=hps.train.fp16_run):
			...
			time_5 = time.time()
			wav = commons.slice_segments(wav, ids_slice * hps.data.hop_length, hps.train.segment_size)  # slice
		...
		if rank == 0:
			if global_step % hps.train.log_interval == 0:
				...
				logger.info([text.item() for text in losses] + [global_step, lr])
